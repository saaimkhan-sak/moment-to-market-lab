...
function renderYouTube(profile, summaries, moment) {
  ...
  document.querySelector('#youtube-summary').textContent = `${profile.club_name}’s verified channel has ${fmt.format(row.video_count)} accessible videos in this archive, stretching from ${row.oldest_published_at.slice(0, 10)} to ${row.newest_published_at.slice(0, 10)}. The title groups below describe what the club publishes; they do not tell us which format performed best.`;
  const formats = Object.entries(row.format_counts).sort((a, b) => b[1] - a[1]);
  document.querySelector('#youtube-formats').innerHTML = `<table class="format-ledger"><thead><tr><th>WHAT THE TITLE SUGGESTS</th><th>VIDEOS</th></tr></thead><tbody>${formats.map(([label, count]) => `<tr><td>${esc(contentFormatLabel(label))}</td><td>${fmt.format(count)}</td></tr>`).join('')}</tbody></table>`;
  const historicalBlock = historical
    ? `<div class="publication-evidence"><h3>What the club published after ${esc(momentLabel(moment).toLowerCase())}</h3><p>An upload appeared after ${fmt.format(historical.qualifying_moments_with_uploads)} qualifying moments in the archive.</p><div class="mini-timeline"><div><b>${fmt.format(historical.official_uploads_by_window.immediate || 0)}</b><span>FIRST 48 HOURS</span></div><div><b>${fmt.format(historical.official_uploads_by_window.short_persistence || 0)}</b><span>NEXT TWO DAYS</span></div><div><b>${fmt.format(historical.official_uploads_by_window.sustained_attention || 0)}</b><span>REST OF WEEK</span></div></div><p class="plain-caveat">These are publication times and surviving public-comment timestamps. They are not reconstructed views, likes, or subscriber counts from the date of the event.</p></div>`
  ...
  document.querySelector('#youtube-videos').innerHTML = `${historicalBlock}<h3>Most-viewed public videos as they stand today</h3><table class="video-ledger"><tbody>${row.top_current_public_view_snapshots.slice(0, 5).map(video => `<tr><td>${fmt.format(Number(video.view_count || 0))}<br>VIEWS NOW</td><td><a href="${esc(video.source_url)}" target="_blank" rel="noreferrer">${esc(video.title)}</a><br><span class="quiet-label">Published ${esc(video.published_at.slice(0, 10))}</span></td></tr>`).join('')}</tbody></table><p class="plain-caveat">These totals were retrieved on ${esc(row.retrieved_at.slice(0, 10))}. They show the public count now, not the path each video took to get there.</p>`;
}
...
function renderMarket(profile, market) {
  ...
  document.querySelector('#market-table').innerHTML = `<table class="market-ledger"><thead><tr><th>MARKET MEASURE</th><th>PUBLIC FIGURE</th></tr></thead><tbody>${[...selected, ...concentrations].map(row => `<tr><td>${esc(marketMetricLabel(row))}<br><span class="quiet-label">${esc(row.reference_period)}${row.fallback_used ? ' · Census estimate used because the employment cell was withheld' : ''}</span></td><td>${esc(formatMetric(row))}</td></tr>`).join('')}</tbody></table>`;
}
...
async function main([profiles, league]) {
  ...
  async function render({clubChanged = false} = {}) {
    ...
    renderPlaybooks(state.profile, state.playbooks, state.moment);
