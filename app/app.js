...
function renderYouTube(profile, summaries, moment) {
  ...
  document.querySelector('#youtube-videos').innerHTML = `${historicalBlock}<h3>Most-viewed public videos as they stand today</h3><table class="video-ledger"><tbody>${row.top_current_public_view_snapshots.slice(0, 5).map(video => `<tr><td><a class="video-thumbnail" href="${esc(video.source_url)}" target="_blank" rel="noreferrer"><img src="${esc(youtubeThumbnail(video))}" alt="Thumbnail for ${esc(video.title)}" loading="lazy"></a><span class="context-number">${esc(fmt.format(Number(video.view_count || 0)))}</span><br>VIEWS NOW</td><td><a href="${esc(video.source_url)}" target="_blank" rel="noreferrer">${esc(video.title)}</a><br><span class="quiet-label">Published ${esc(video.published_at.slice(0, 10))}</span></td></tr>`).join('')}</tbody></table><p class="plain-caveat">These totals were retrieved on ${esc(row.retrieved_at.slice(0, 10))}. They show the public count now, not the path each video took to get there.</p>`;
}
